...
<html lang="en">
    ...
    <body>
        ...
        <header class="header">
            ...
            $filter = $_GET['subdit'];
            if ($_GET['by'] == 'jenis' && $_GET['subdit'] == 'Kota Magelang') {
            ...
            }elseif (($_GET['subdit'] == 'Kota Magelang' && $_GET['by'] == 'bulan')) {
...
<?php
                        ...
                        if (in_array($filter,$jenis)){
                           ...
                           $result = mysqli_query($conn,"SELECT * FROM `Kasus` WHERE `Lokasi Kejadian` LIKE'Kota Magelang%' AND `Jenis Tindak Pidana` = '$filter'");    
                        }elseif (in_array($filter, $bulan)){
                            $monthName = $filter ; // Indonesian month name
		                    $filter =  array_search($monthName, $bulan) + 1;
                            $result = mysqli_query($conn,"SELECT * FROM `Kasus` WHERE `Lokasi Kejadian` LIKE'Kota Magelang%' AND `Waktu Kejadian`LIKE '%2023%$filter-%' ");
                        }else{
                            $result = mysqli_query($conn,"SELECT * FROM `Kasus` WHERE `Lokasi Kejadian` LIKE'Kota Magelang%'");
                        }
?>

<main>
    <div class="map-container">
    <h1><strong>Peta Kriminalis Kota Magelang</strong></h1>
        <a href="/detail.php">
        ...
        elseif ($_GET['subdit']=='Kota Magelang' or ($_GET['subdit']=='Kota Magelang' and$_GET['by'] == 'jenis') or ($_GET['subdit']=='Kota Magelang' and$_GET['by'] == 'bulan') ) {echo '';}
        ?>


        <?php
        // bulan
        if (in_array($_GET['subdit'],$bulan) or ($_GET['subdit']=='Kota Magelang' and$_GET['by'] == 'bulan')){
        echo "<table>
          <thead>
            <tr>
                <th>No.</th>
      ...
      //			echo  $row["ID"]. $row["Nama Kasus"].  $row["Jenis Tindak Pidana"].   $row["Waktu Kejadian"].  $row["Lokasi Kejadian"]. "<br>";
        ...
        }elseif (($_GET['by'] == 'jenis' and $_GET['subdit'] == 'Kota Magelang') or in_array($_GET['subdit'],$jenis)) {
        // jenis
        echo "<table>
          <thead>
            <tr>
...
?>        
        ...
        }).fitBounds([[-7.51123554790156,110.1614294474228],[-7.429649414753813,110.27584902439831]]);
        ...
        function style_Batas_Kecamatan_BIG_0_0(feature) {
            if (feature.properties['OBJECTID'] >= 3534.000000 && feature.properties['OBJECTID'] <= 3534.666667 ) {
                return {
            ...
            if (feature.properties['OBJECTID'] >= 3534.666667 && feature.properties['OBJECTID'] <= 3535.333333 ) {
                return {
                pane: 'pane_Batas_Kecamatan_BIG_0',
                opacity: 1,
                color: 'rgba(35,35,35,1.0)',
            ...
            if (feature.properties['OBJECTID'] >= 3535.333333 && feature.properties['OBJECTID'] <= 3536.000000 ) {
                return {
                pane: 'pane_Batas_Kecamatan_BIG_0',
                opacity: 1,
                color: 'rgba(35,35,35,1.0)',
...
<div class="title">
<h1><strong><?php if (in_array($_GET['subdit'],$jenis) or ($_GET['subdit'] == 'Kota Magelang' and $_GET['by'] == 'jenis')){echo 'Berdasarkan Bidang Tindak Pidana';}elseif (in_array($_GET['subdit'],$bulan) or ($_GET['by'] == 'bulan' and $_GET['subdit'] == 'Kota Magelang')  ){echo 'Berdasarkan Waktu';} ?></strong></h1>
      </div>
<br>
</main>
