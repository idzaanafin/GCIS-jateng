...
<html lang="en">
    ...
    <body>
        ...
        <header class="header">
            ...
            $filter = $_GET['subdit'];
            if ($_GET['by'] == 'jenis' && $_GET['subdit'] == 'Kota Salatiga') {
            ...
            }elseif (($_GET['subdit'] == 'Kota Salatiga' && $_GET['by'] == 'bulan')) {
...
<?php
                        ...
                        if (in_array($filter,$jenis)){
                           ...
                           $result = mysqli_query($conn,"SELECT * FROM `Kasus` WHERE `Lokasi Kejadian` LIKE'Kota Salatiga%' AND `Jenis Tindak Pidana` = '$filter'");    
                        }elseif (in_array($filter, $bulan)){
                            $monthName = $filter ; // Indonesian month name
		                    $filter =  array_search($monthName, $bulan) + 1;
                            $result = mysqli_query($conn,"SELECT * FROM `Kasus` WHERE `Lokasi Kejadian` LIKE'Kota Salatiga%' AND `Waktu Kejadian`LIKE '%2023%$filter-%' ");
                        }else{
                            $result = mysqli_query($conn,"SELECT * FROM `Kasus` WHERE `Lokasi Kejadian` LIKE'Kota Salatiga%'");
                        }
?>

<main>
    <div class="map-container">
    <h1><strong>Peta Kriminalis Kota Salatiga</strong></h1>
        <a href="/detail.php">
        ...
        elseif ($_GET['subdit']=='Kota Salatiga' or ($_GET['subdit']=='Kota Salatiga' and$_GET['by'] == 'jenis') or ($_GET['subdit']=='Kota Salatiga' and$_GET['by'] == 'bulan') ) {echo '';}
        ?>


        <?php
        // bulan
        if (in_array($_GET['subdit'],$bulan) or ($_GET['subdit']=='Kota Salatiga' and$_GET['by'] == 'bulan')){
        echo "<table>
          <thead>
            <tr>
                <th>No.</th>
      ...
      //			echo  $row["ID"]. $row["Nama Kasus"].  $row["Jenis Tindak Pidana"].   $row["Waktu Kejadian"].  $row["Lokasi Kejadian"]. "<br>";
        ...
        }elseif (($_GET['by'] == 'jenis' and $_GET['subdit'] == 'Kota Salatiga') or in_array($_GET['subdit'],$jenis)) {
        // jenis
        echo "<table>
          <thead>
            <tr>
...
?>        
        ...
        }).fitBounds([[-7.397953773681802,110.43368368452067],[-7.285193812188814,110.59182265490719]]);
        ...
        function style_Batas_Kecamatan_BIG_0_0(feature) {
            if (feature.properties['OBJECTID'] >= 288.000000 && feature.properties['OBJECTID'] <= 4489.500000 ) {
                return {
            ...
            if (feature.properties['OBJECTID'] >= 4489.500000 && feature.properties['OBJECTID'] <= 5891.000000 ) {
                return {
                pane: 'pane_Batas_Kecamatan_BIG_0',
                opacity: 1,
                color: 'rgba(35,35,35,1.0)',
            ...
            if (feature.properties['OBJECTID'] >= 5891.000000 && feature.properties['OBJECTID'] <= 6151.250000 ) {
                return {
                pane: 'pane_Batas_Kecamatan_BIG_0',
                opacity: 1,
                color: 'rgba(35,35,35,1.0)',
            ...
            if (feature.properties['OBJECTID'] >= 6151.250000 && feature.properties['OBJECTID'] <= 6929.000000 ) {
                return {
                pane: 'pane_Batas_Kecamatan_BIG_0',
                opacity: 1,
                color: 'rgba(35,35,35,1.0)',
...
<div class="title">
<h1><strong><?php if (in_array($_GET['subdit'],$jenis) or ($_GET['subdit'] == 'Kota Salatiga' and $_GET['by'] == 'jenis')){echo 'Berdasarkan Bidang Tindak Pidana';}elseif (in_array($_GET['subdit'],$bulan) or ($_GET['by'] == 'bulan' and $_GET['subdit'] == 'Kota Salatiga')  ){echo 'Berdasarkan Waktu';} ?></strong></h1>
      </div>
<br>
</main>
