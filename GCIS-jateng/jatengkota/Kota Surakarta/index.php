...
<html lang="en">
    ...
    <body>
        ...
        <header class="header">
            ...
            $filter = $_GET['subdit'];
            if ($_GET['by'] == 'jenis' && $_GET['subdit'] == 'Kota Surakarta') {
            ...
            }elseif (($_GET['subdit'] == 'Kota Surakarta' && $_GET['by'] == 'bulan')) {
...
<?php
                        ...
                        if (in_array($filter,$jenis)){
                           ...
                           $result = mysqli_query($conn,"SELECT * FROM `Kasus` WHERE `Lokasi Kejadian` LIKE'Kota Surakarta%' AND `Jenis Tindak Pidana` = '$filter'");    
                        }elseif (in_array($filter, $bulan)){
                            $monthName = $filter ; // Indonesian month name
		                    $filter =  array_search($monthName, $bulan) + 1;
                            $result = mysqli_query($conn,"SELECT * FROM `Kasus` WHERE `Lokasi Kejadian` LIKE'Kota Surakarta%' AND `Waktu Kejadian`LIKE '%2023%$filter-%' ");
                        }else{
                            $result = mysqli_query($conn,"SELECT * FROM `Kasus` WHERE `Lokasi Kejadian` LIKE'Kota Surakarta%'");
                        }
?>

<main>
    <div class="map-container">
    <h1><strong>Peta Kriminalis Kota Surakarta</strong></h1>
        <a href="/detail.php">
        ...
        elseif ($_GET['subdit']=='Kota Surakarta' or ($_GET['subdit']=='Kota Surakarta' and$_GET['by'] == 'jenis') or ($_GET['subdit']=='Kota Surakarta' and$_GET['by'] == 'bulan') ) {echo '';}
        ?>


        <?php
        // bulan
        if (in_array($_GET['subdit'],$bulan) or ($_GET['subdit']=='Kota Surakarta' and$_GET['by'] == 'bulan')){
        echo "<table>
          <thead>
            <tr>
                <th>No.</th>
      ...
      //			echo  $row["ID"]. $row["Nama Kasus"].  $row["Jenis Tindak Pidana"].   $row["Waktu Kejadian"].  $row["Lokasi Kejadian"]. "<br>";
        ...
        }elseif (($_GET['by'] == 'jenis' and $_GET['subdit'] == 'Kota Surakarta') or in_array($_GET['subdit'],$jenis)) {
        // jenis
        echo "<table>
          <thead>
            <tr>
...
?>        
        ...
        }).fitBounds([[-7.603517424114403,110.77551434228612],[-7.521931290966658,110.88993391926162]]);
        ...
        function style_Batas_Kecamatan_BIG_0_0(feature) {
            if (feature.properties['OBJECTID'] >= 581.000000 && feature.properties['OBJECTID'] <= 1891.400000 ) {
                return {
            ...
            if (feature.properties['OBJECTID'] >= 1891.400000 && feature.properties['OBJECTID'] <= 2850.200000 ) {
                return {
                pane: 'pane_Batas_Kecamatan_BIG_0',
                opacity: 1,
                color: 'rgba(35,35,35,1.0)',
            ...
            if (feature.properties['OBJECTID'] >= 2850.200000 && feature.properties['OBJECTID'] <= 3856.600000 ) {
                return {
                pane: 'pane_Batas_Kecamatan_BIG_0',
                opacity: 1,
                color: 'rgba(35,35,35,1.0)',
            ...
            if (feature.properties['OBJECTID'] >= 3856.600000 && feature.properties['OBJECTID'] <= 4948.200000 ) {
                return {
                pane: 'pane_Batas_Kecamatan_BIG_0',
                opacity: 1,
                color: 'rgba(35,35,35,1.0)',
            ...
            if (feature.properties['OBJECTID'] >= 4948.200000 && feature.properties['OBJECTID'] <= 5801.000000 ) {
                return {
                pane: 'pane_Batas_Kecamatan_BIG_0',
                opacity: 1,
                color: 'rgba(35,35,35,1.0)',
...
<div class="title">
<h1><strong><?php if (in_array($_GET['subdit'],$jenis) or ($_GET['subdit'] == 'Kota Surakarta' and $_GET['by'] == 'jenis')){echo 'Berdasarkan Bidang Tindak Pidana';}elseif (in_array($_GET['subdit'],$bulan) or ($_GET['by'] == 'bulan' and $_GET['subdit'] == 'Kota Surakarta')  ){echo 'Berdasarkan Waktu';} ?></strong></h1>
      </div>
<br>
</main>
